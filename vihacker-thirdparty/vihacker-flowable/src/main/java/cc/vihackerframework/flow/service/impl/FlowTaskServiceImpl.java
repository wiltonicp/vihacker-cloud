...
import cc.vihackerframework.flow.entity.enums.FlowComment;
import cc.vihackerframework.flow.factory.ViHackerFlowServiceFactory;
import cc.vihackerframework.flow.feign.ISysUserProvider;
import cc.vihackerframework.flow.service.IFlowTaskService;
import cc.vihackerframework.flow.util.FlowableUtils;
...
@Slf4j
@Service
public class FlowTaskServiceImpl extends ViHackerFlowServiceFactory implements IFlowTaskService {
    ...
    @Override
    public ViHackerApiResult complete(FlowTaskVo taskVo) {
        Task task = taskService.createTaskQuery().taskId(taskVo.getTaskId()).singleResult();
        if (Objects.isNull(task)) {
            return ViHackerApiResult.failed("任务不存在");
        }
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        if (taskService.createTaskQuery().taskId(flowTaskVo.getTaskId()).singleResult().isSuspended()) {
            throw new ViHackerRuntimeException("任务处于挂起状态");
        }
        // 当前任务 task
        ...
        if (parentUserTaskList == null || parentUserTaskList.size() == 0) {
            throw new ViHackerRuntimeException("当前节点为初始任务节点，不能驳回");
        }
        // 获取活动 ID 即节点 Key
        ...
        if (targetIds.size() > 1 && currentIds.size() > 1) {
            throw new ViHackerRuntimeException("任务出现多对多情况，无法撤回");
        }
        ...
        try {
            ...
            throw new ViHackerRuntimeException("未找到流程实例，流程可能已发生变化");
        } catch (FlowableException e) {
            throw new ViHackerRuntimeException("无法取消或开始活动");
        }
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        if (!isSequential) {
            throw new ViHackerRuntimeException("当前节点相对于目标节点，不属于串行关系，无法回退");
        }
     ...
     */
    @Override
    public ViHackerApiResult findReturnTaskList(FlowTaskVo flowTaskVo) {
        // 当前任务 task
        Task task = taskService.createTaskQuery().taskId(flowTaskVo.getTaskId()).singleResult();
        // 获取流程定义信息
        ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionId(task.getProcessDefinitionId()).singleResult();
        ...
        return ViHackerApiResult.data(userTaskList);
    }
     ...
     */
    @Override
    public ViHackerApiResult myProcess(Integer pageNum, Integer pageSize) {
...
//            }
        ...
        return ViHackerApiResult.data(page);
    }
     ...
     */
    @Override
    public ViHackerApiResult stopProcess(FlowTaskVo flowTaskVo) {
        ...
        if (Objects.nonNull(bpmnModel)) {
            ...
            if (CollectionUtils.isNotEmpty(endNodes)) {
                Authentication.setAuthenticatedUserId(current.getUserId().toString());
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
     ...
     */
    @Override
    public ViHackerApiResult revokeProcess(FlowTaskVo flowTaskVo) {
        ...
        for (HistoricTaskInstance hti : htiList) {
            if (current.getUserId().toString().equals(hti.getAssignee())) {
                myTaskId = hti.getId();
                myTask = hti;
                break;
            }
        ...
        if (null == myTaskId) {
            throw new ViHackerRuntimeException("该任务非当前用户提交，无法撤回");
        }
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
     ...
     */
    @Override
    public ViHackerApiResult todoList(Integer pageNum, Integer pageSize) {
        Page<FlowTaskDto> page = new Page<>();
        Long userId = UserContext.current().getUserId();
...
//                .taskAssignee(userId.toString())
        ...
        for (Task task : taskList) {
            ...
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                    .processInstanceId(task.getProcessInstanceId())
                    .singleResult();
            SysUser startUser = (SysUser) sysUserProvider.selectUserById(Long.parseLong(historicProcessInstance.getStartUserId())).getData();
            flowTask.setStartUserId(startUser.getUsername());
            flowTask.setStartUserName(startUser.getUsername());
            flowTask.setStartDeptName(startUser.getDeptName());
            flowList.add(flowTask);
        }
     ...
     */
    @Override
    public ViHackerApiResult finishedList(Integer pageNum, Integer pageSize) {
        Page<FlowTaskDto> page = new Page<>();
        Long userId = UserContext.current().getUserId();
...
//        result.put("finished",true);
     ...
     */
    @Override
    public ViHackerApiResult getFlowViewer(String procInsId) {
        ...
        return ViHackerApiResult.data(flowViewerList);
    }
     ...
     */
    @Override
    public ViHackerApiResult processVariables(String taskId) {
        // 流程变量
        HistoricTaskInstance historicTaskInstance = historyService.createHistoricTaskInstanceQuery().includeProcessVariables().finished().taskId(taskId).singleResult();
        if (Objects.nonNull(historicTaskInstance)) {
            return ViHackerApiResult.data(historicTaskInstance.getProcessVariables());
        } else {
            Map<String, Object> variables = taskService.getVariables(taskId);
            return ViHackerApiResult.data(variables);
        }
    }
